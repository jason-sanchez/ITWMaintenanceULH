...
Module Module1
    ...
    Private Sub ProcessRadios(ByRef conn As SqlConnection)
        ...
        sql = "SELECT r.radioValue, r.radioNo, el.[ID] as EvalElID, "
        ...
        sql &= "FROM [100EvalRadio] r "
        sql &= "INNER JOIN [100EvalEl] el ON r.elementID = el.[ID] "
        ...
        ' around this, keep track of the currentEvalElID and build
        ' the options using a local variable.  If the currentEvalElID
        ' doesn't match the current records's EvalElID and the options
        ' variable contains data, fire an update.
        Dim currentEvalElID As Integer = 0
        Dim options(19) As String

        Console.WriteLine("Processing " & ds.Tables(0).Rows.Count & " records...")
        For Each dr In ds.Tables(0).Rows
            If currentEvalElID <> dr.Item("EvalElID") Then
                If currentEvalElID > 0 Then
                    UpdateOptions(currentEvalElID, conn, options)
                End If
                ...
                ' Grab the new EvalElID
                currentEvalElID = dr("EvalElID")
            End If
    ...
    Private Sub ProcessCombos(ByRef conn As SqlConnection)
        ...
        sql = "SELECT c.optionValue, c.comboNo, el.[ID] as EvalElID, "
        sql &= "el.type01, el.type02, el.type03, el.type04, el.type05, "
        sql &= "el.type06, el.type07, el.type08, el.type09, el.type10, "
        sql &= "el.type11, el.type12, el.type13, el.type14, el.type15, "
        sql &= "el.type16, el.type17, el.type18, el.type19, el.type20, "
        ...
        sql &= "el.Field11Options, el.Field12Options, el.Field13Options, el.Field14Options, el.Field15Options, "
        sql &= "el.Field16Options, el.Field17Options, el.Field18Options, el.Field19Options, el.Field20Options "
        sql &= "FROM [100EvalCombo] c "
        sql &= "INNER JOIN [100EvalEl] el ON c.elementID = el.[ID] "
    ...
    Private Sub UpdateOptions(ByVal currentEvalElID As Integer, ByRef conn As SqlConnection, ByRef options As String())
        Dim sql As String
        ...
        sql &= "WHERE [ID] = " & currentEvalElID & " "

        Console.WriteLine("  Updating Form ([100EvalEl].ID): " & currentEvalElID)
