...
Partial Class Evals_Form_Edit
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ElseIf Not frmEdit.CurrentMode = FormViewMode.ReadOnly Then
            ...
            Dim theForm As EvalForm = GetForm()
            ...
            theForm.EClass = CType(frmEdit.FindControl("EClassDropDownList"), DropDownList).SelectedValue
            theForm.Calculated = CType(frmEdit.FindControl("CalculatedRadioButton"), RadioButton).Checked
            theForm.Calculation = CType(frmEdit.FindControl("CalculationTextBox"), TextBox).Text
            theForm.NoResult = CType(frmEdit.FindControl("NoResultRadioButton"), RadioButton).Checked
            ...
            theForm.Outcome = CType(frmEdit.FindControl("OutcomeCheckBox"), CheckBox).Checked
    ...
    Private Sub ApplyAuthorizationRules()
        If Evaluation.CanEditObject Then
            ...
            End If
        Else
            Me.lblHeader.Text = "View Eval Form"
    ...
    Protected Sub frmEdit_DataBound(sender As Object, e As EventArgs) Handles frmEdit.DataBound
        If frmEdit.CurrentMode = FormViewMode.ReadOnly AndAlso Not frmEdit.DataItem Is Nothing Then
            ...
            Dim FormInfo As ReadOnlyEvalForm = DirectCast(frmEdit.DataItem, ReadOnlyEvalForm)
    ...
    Protected Sub frmEdit_ItemCreated(ByVal sender As Object, ByVal e As System.EventArgs) Handles frmEdit.ItemCreated
        ...
        If frmEdit.DefaultMode = DetailsViewMode.Insert Then
            Try
                ...
                CType(frmEdit.FindControl("EClassDropDownList"), DropDownList).SelectedValue = obj.EClass
                CType(frmEdit.FindControl("CalculatedRadioButton"), RadioButton).Checked = obj.Calculated
                CType(frmEdit.FindControl("CalculationTextBox"), TextBox).Text = obj.Calculation
                CType(frmEdit.FindControl("NoResultRadioButton"), RadioButton).Checked = obj.NoResult
                ...
                CType(frmEdit.FindControl("OutcomeCheckBox"), CheckBox).Checked = obj.Outcome
        ...
        ' Automatically check the "No Calculation" radio button if the form is not calculated and doesn't have a result
        CType(frmEdit.FindControl("NoCalculationRadioButton"), RadioButton).Checked = (Not obj.Calculated AndAlso Not obj.NoResult)
        ...
        If Not frmEdit.CurrentMode = FormViewMode.ReadOnly Then
            ...
            For Each lookupValue In obj.LookupList
                ' 10/18/2016 Matt - Ignore lookups that are on fields that are outside of the 1-20 range.
                ' (An example is field 999 is used to reference the calulation result)
                If lookupValue.FormField > 0 AndAlso lookupValue.FormField <= 20 Then
    ...
    Protected Sub EvalFormDataSource_SelectObject(ByVal sender As Object, ByVal e As Csla.Web.SelectObjectArgs) Handles EvalFormDataSource.SelectObject
        If Me.frmEdit.CurrentMode = FormViewMode.ReadOnly Then
            e.BusinessObject = ReadOnlyEvalForm.GetEvalFormInfo(Request("EvalID"))
        Else
            e.BusinessObject = GetForm()
        End If
    End Sub

    Protected Sub EvalFormDataSource_InsertObject(ByVal sender As Object, ByVal e As Csla.Web.InsertObjectArgs) Handles EvalFormDataSource.InsertObject
        Dim obj As EvalForm = GetForm()
        Csla.Data.DataMapper.Map(e.Values, obj)
        SaveForm(obj)
    End Sub

    Protected Sub EvalFormDataSource_UpdateObject(ByVal sender As Object, ByVal e As Csla.Web.UpdateObjectArgs) Handles EvalFormDataSource.UpdateObject
        Dim obj As EvalForm = GetForm()
        Csla.Data.DataMapper.Map(e.Values, obj, "EvalID")
        SaveForm(obj)
    End Sub

    Protected Sub EvalFormDataSource_DeleteObject(ByVal sender As Object, ByVal e As Csla.Web.DeleteObjectArgs) Handles EvalFormDataSource.DeleteObject
        Try
            EvalForm.DeleteEvalForm(Request("EvalID"))
            Session("currentObject") = Nothing
            GoBack()
    ...
    Private Sub GoBack()
        ...
        ' If we are editing an existing form, Request("EvalID") will hold
        ' the Eval ID we're looking for.  But if we've just added a new form,
        ' we'll need to pull the Eval ID from the business object.
        If Not IsNothing(Session("currentObject")) AndAlso TypeOf Session("currentObject") Is EvalForm Then
            evalID = DirectCast(Session("currentObject"), EvalForm).EvalID.ToString()
        End If

        Response.Redirect("Eval_Landing.aspx?ShowEvalID=" & evalID)
    End Sub

    Private Function GetForm() As EvalForm
        Dim businessObject As Object = Session("currentObject")

        If IsNothing(businessObject) OrElse Not TypeOf businessObject Is EvalForm Then
            Try
                Dim idString As String = Request("EvalID")
                Dim parentID As String = Request("ParentID")

                If Not String.IsNullOrEmpty(idString) Then
                    businessObject = EvalForm.GetEvalForm(idString)
                ElseIf Not String.IsNullOrEmpty(parentID) Then
                    businessObject = EvalForm.NewEvalForm(parentID)
                Else
        ...
        Return CType(businessObject, EvalForm)
    End Function

    Private Sub SaveForm(ByVal theForm As EvalForm)
        Try
    ...
    Protected Sub CopyButton_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Session("CopiedEvalForm") = Session("currentObject")
        Response.Redirect("Eval_Landing.aspx?ShowEvalID=" & CType(Session("CopiedEvalForm"), EvalForm).EvalID)
    End Sub
    ...
    Private Sub SetValues(ByRef FormInfo As ReadOnlyEvalForm, ByRef i As Int16, ByRef FieldLabel As String, ByRef FieldType As String, ByRef FieldOptions As String, ByRef FieldHelpText As String, ByRef FieldValidation As String)
        Select Case i
            Case 1
